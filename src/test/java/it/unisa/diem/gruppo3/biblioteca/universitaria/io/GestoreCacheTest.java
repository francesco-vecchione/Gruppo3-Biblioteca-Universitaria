...
public class GestoreCacheTest {
    ...
    public void tearDown() {
       //Alla fine di ogni test viene eliminato il file di cache.
        File fLibri = new File("archivioLibriCache");
        if(fLibri.exists()) fLibri.delete();

        File fUtenti = new File("archivioUtentiCache");
        if(fUtenti.exists()) fUtenti.delete();

        File fPrestiti = new File("archivioPrestitiCache");
        if(fPrestiti.exists()) fPrestiti.delete();
    }
